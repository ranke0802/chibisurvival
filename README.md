...
## 릴리즈 노트
### v0.01
- 화면 정보량 개선을 위해 월드 렌더 배율 `60%` 적용
- 캐릭터/몬스터/투사체를 동일 월드 스케일 기준으로 출력
- 잔렉 완화를 위한 렌더링 컬링(화면 밖 오브젝트 스킵) 적용
- 배경/오버레이 렌더를 뷰포트 범위 중심으로 최적화
- 공격 타겟 탐색 로직에서 과도한 정렬 사용을 줄이고 최근접 탐색 경량화
- 몬스터/보석/파티클/데미지텍스트 상한 도입으로 후반 프레임 드랍 완화
- 캔버스 DPR 상한 조정으로 고해상도 환경 GPU 부하 감소
